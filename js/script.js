...
menuButton.addEventListener('click', function () {
  ...
  menuButton.classList.toggle('_open');
...
if (menuLinks.length > 0) {
  var onMenuLinkClick = function onMenuLinkClick(e) {
    ...
    if (menuLink.dataset["goto"] && document.querySelector(menuLink.dataset["goto"])) {
      ...
      if (menuButton.classList.contains('_open')) {
        menuButton.setAttribute('aria-expanded', !(menuButton.getAttribute('aria-expanded') === 'true'));
        document.body.classList.remove('_lock');
        menuButton.classList.remove('_open');
...
if (spoilersArray.length > 0) {
  ...
  if (spoilersMedia.length > 0) {
    ...
    mediaQueries.forEach(function (breakpoint) {
      ...
      var matchMedia = window.matchMedia(paramsArray[0]); // Объекты с нужным условиям
...
var unlock = true;
var timeout = 600; // Ссылки для открытия

if (popupLinks.length > 0) {
  var _loop2 = function _loop2(_index) {
    var popupLink = popupLinks[_index];
...
var popupCloselcon = document.querySelectorAll('.close-popup');

if (popupCloselcon.length > 0) {
  var _loop3 = function _loop3(_index2) {
    var el = popupCloselcon[_index2];
  ...
  for (var _index2 = 0; _index2 < popupCloselcon.length; _index2++) {
    _loop3(_index2);
  }
}
...
;
[].forEach.call(document.querySelectorAll('[data-telephone]'), function (input) {
  var keyCode;
...
;
var swiper1 = new Swiper('.examples__slider', {
  navigation: {
...
lightGallery(document.getElementById('animated-thumbnials'), {
  ...
  selector: '.examples__slider-slide'
});